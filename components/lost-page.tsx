...
export default function LostClient() {
  ...
  const filtered = useMemo(() => {
    ...
    let arr = items.filter((i) => {
      if (activeCat) {
        const cn = (i as CardItemInput & { category?: { name?: string } }).category?.name ?? "";
  ...
  function applyToUrl() {
    ...
    router.replace(`/lost${sp.toString() ? "?" + sp.toString() : ""}`, { scroll: false });
  }
  ...
  function resetFilters() {
    ...
    setActiveCat("");
    setSort("newest");
    router.replace("/lost", { scroll: false });
        ...
        {activeCat && (
          <span className="inline-flex items-center gap-1.5 rounded-full bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-800 ring-1 ring-indigo-200">
            ...
            <button
              ...
              className="ml-1 grid h-4 w-4 place-items-center rounded-full bg-indigo-100 text-indigo-700 transition hover:bg-indigo-200"
            >
              <X size={11} />
            </button>
          </span>
